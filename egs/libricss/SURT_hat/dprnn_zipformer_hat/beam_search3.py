...
def greedy_search_batch(
    ...
    for (t, batch_size) in enumerate(batch_size_list):
        ...
        # Stack encoder and decoder outputs for both channels.
        # NOTE: We use "(b c)" instead of "(c b)" because pack_padded_sequence sorts
        # the batch in descending order, so channels from the same utterance would be
        # grouped together. This means that if the batch originally was
        # [utt1_ch1, utt2_ch1, ..., utt1_ch2, utt2_ch2, ...], it is now
        # [utt1_ch1, utt1_ch2, utt2_ch1, utt2_ch2, ...].
        current_aux_encoder_out = rearrange(
            current_aux_encoder_out,
            "(b c) t s d -> b t s (c d)",
            c=model.num_channels,
        )
        current_aux_decoder_out = rearrange(
        ...
        aux_logits = model.aux_joiner(
            current_aux_encoder_out,
            current_aux_decoder_out,
            project_input=False,
        )

        # Separate logits for each channel and stack them along batch axis (grouping
        # together the channels from the same utterance)
        aux_logits = rearrange(
            aux_logits, "b t s (c d) -> (b c) t s d", c=model.num_channels
        )
