...
def get_aux_joint_encoder_layer(params: AttributeDict) -> nn.Module:
    ...
    if params.use_aux_joint_encoder_layer == "linear":
        aux_encoder_dim = int(params.aux_encoder_dim.split(",")[-1])
        aux_joint_layer = nn.Sequential(
            Rearrange("(c b) t d -> b t (c d)", c=params.num_channels),
            nn.Linear(
                params.num_channels * aux_encoder_dim,
                params.num_channels * aux_encoder_dim,
            ),
            nn.ReLU(),
            Rearrange("b t (c d) -> (c b) t d", c=params.num_channels),
        )
    elif params.use_aux_joint_encoder_layer == "lstm":
        aux_encoder_dim = int(params.aux_encoder_dim.split(",")[-1])
        aux_joint_layer = nn.Sequential(
            Rearrange("(c b) t d -> b t (c d)", c=params.num_channels),
            ScaledLSTM(
                input_size=params.num_channels * aux_encoder_dim,
                hidden_size=params.num_channels * aux_encoder_dim,
                num_layers=1,
                bias=True,
                batch_first=True,
                dropout=0.0,
        ...
        raise ValueError(
            f"Unknown joint encoder layer type: {params.use_aux_joint_encoder_layer}"
        )
    return aux_joint_layer
...
def compute_loss(
    ...
    if params.use_aux_encoder:
        ...
        # Repeat each speaker ID for each token in the corresponding text.
        speakers = [
            [
                speaker
...
def run(rank, world_size, args):
    ...
    train_cuts = libricss.lsmix_cuts(rvb_affix="norvb", type_affix="v1", sources=True)
    train_cuts_ov40 = libricss.lsmix_cuts(
        rvb_affix="norvb", type_affix="ov40", sources=True
    )
    # dev_cuts = libricss.libricss_cuts(split="dev", type="ihm-mix")
    # We cannot use LibriCSS if we are using max speakers < 8
    dev_cuts = libricss.lsmix_dev_cuts(rvb_affix="norvb", type_affix="v1").subset(
        first=200
    )
    ...
    train_dl_ov40 = libricss.train_dataloaders(
        train_cuts_ov40, text_delimiter=text_delimiter
    )
    valid_dl = libricss.valid_dataloaders(dev_cuts, text_delimiter=text_delimiter)
