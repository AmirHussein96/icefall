...
torchrun --nproc_per_node 1 ./whisper/inference.py   --exp-dir whisper/exp_large_v3_30   --model-name large-v3   --epoch 30  --manifest-dir data/fbank_whisper    --max-duration 300 --avg 1 --remove-whisper-encoder-input-length-restriction False
...
import whisper
from asr_datamodule import SccAsrDataModule
...
def remove_short_and_long_utt(c):
        ...
        # tokens = c.supervisions[0].text.replace("|", ""), out_type=str
...
def decode_one_batch(
    ...
    hyps = [clean(text).split() for text in hyps]
    print(hyps)
    ...
    for batch_idx, batch in enumerate(dl):
        ...
        for lm_scale, hyps in hyps_dict.items():
            ...
            for cut_id, hyp_words, ref_text in zip(cut_ids, hyps, texts):
                ref_words =  clean(ref_text).split()
...
def main():
    parser = get_parser()
    SccAsrDataModule.add_arguments(parser)
    args = parser.parse_args()
    args.exp_dir = Path(args.exp_dir)
    
    params = get_params()
    params.update(vars(args))
    params.res_dir = params.exp_dir
    params.suffix = f"epoch-{params.epoch}-beam{params.beam_size}-avg-{params.avg}"
    setup_logger(
    ...
    scc = SccAsrDataModule(args)
    valid_cuts = scc.valid_cuts()
    valid_cuts = valid_cuts.filter(remove_short_and_long_utt)
    valid_dl = scc.valid_dataloaders(valid_cuts)
    
    test_cuts = scc.test_cuts()
    test_cuts = test_cuts.filter(remove_short_and_long_utt)
    test_dl = scc.valid_dataloaders(test_cuts)
